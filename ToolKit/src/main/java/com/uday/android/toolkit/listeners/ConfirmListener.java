	...
	public void onClick(final AlertDialog utils){
		...
		switch(fragment.mOption){
			case fragment.SELECTED_REPACK:
				Object[] obj=DialogUtils.showTermDialog(fragment.getContext(),"Repack img","Repacking project please wait...","open folder","finish");
			...
			case fragment.SELECTED_INSTALL:
				fragment.pDialog=new ProgressDialog(fragment.getContext()).show(fragment.getContext(),"Installing","Installing selected "+fragment.choosen+"\nplease wait...",false,false);
				new Thread(fragment.executer).start();
				break;
			case fragment.SELECTED_BACKUP:
				fragment.pDialog=new ProgressDialog(fragment.getContext()).show(fragment.getContext(),"Backup","Backup process in progress please wait...",false,false);
				new Thread(fragment.executer).start();
				break;
			case fragment.SELECTED_RESTORE:
				fragment.pDialog=new ProgressDialog(fragment.getContext()).show(fragment.getContext(),"Restoring","Restoring "+fragment.BLOCK_NAME.toUpperCase()+".\nplease wait...",false,false);
				new Thread(fragment.executer).start();
				break;
